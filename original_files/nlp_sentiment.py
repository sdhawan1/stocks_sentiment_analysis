...
lmdict = {'Negative': ['abandon', 'abandoned', 'abandoning', 'abandonment', 'abandonments', 'abandons', 'abdicated',
                       ...
                       'wrongdoings', 'wrongful', 'wrongfully', 'wrongly','fell', 'short','down'],
...
def sentiment(text):
    ...
    #final_words = re.findall(r'\b([a-zA-Z]+n\'t|[a-zA-Z]+\'s|[a-zA-Z]+)\b', text.lower())
    tkf = re.findall(r'\b([a-zA-Z]+n\'t|[a-zA-Z]+\'s|[a-zA-Z]+)\b', text.lower())
    print (tkf)
    #tkf =[]

    #removing stop words
    #for i in range(0,len(final_words)):
    #    if len(final_words[i])>2:
    #        tkf.append(final_words[i])

    #tkf = [w for w in tkf if not w in stopwords.words("english")]
...
#order of the words stay
    for i in range(0, len(tkf)):
        #print ("each",tkf[i])
        if tkf[i] in lmdict['Negative']:
            negwords +=1
            negative_words.append(tkf[i])
        if tkf[i] in lmdict['Positive']:
            if i >=3:
                if negated(tkf[i]) or negated(tkf[i-1]) or negated(tkf[i-2]):
                    negwords+=1
                    negative_words.append(tkf[i])
                else:
                    poswords+=1
                    positive_words.append(tkf[i])
            if i==2:
                if negated(tkf[i]) or negated(tkf[i-1] ):
                    negwords+=1
                    negative_words.append(tkf[i])
                else:
                    poswords+=1
    ...
    results = [negwords,poswords,negative_words, positive_words]
    #print (results)
    return results
...
"""
    1. fetch a list of "rss feeds" from cnbc.com associated with our ticker.
    2. each ticker has an attached article.
        pull the attached article, probably for sentiment analysis.
"""
def rss_feed(ticker):
    ticker = ticker.lower()
    ...
    for link in links:
        print ("https:"+link) #check the link
    ...
    '''
        here, take the found links and crawl the text from their associated articles?
    '''
    #print (total_items[1][0].title.text)
    #print (total_items[0][].title) #check items
    dictionaries = []
    ...
    '''
        here, start the sentiment analysis?
        ** only on the title and description fields...
    '''
    #sentiment =[]
...
schedule.every(5).minutes.do(rss_feed)
